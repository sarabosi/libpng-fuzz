...
/*                                                                     */
/* Compile with:                                                       */
/*   afl-clang-fast -o harness_persistant harness_persistant.c -lpng  */
/*                                                                     */
/* The __AFL_LOOP(N) macro makes the process handle N inputs before    */
/* forking a fresh child, dramatically reducing fork() overhead.       */
/* __AFL_INIT() moves the fork point past one-time initialisation.     */
/* ------------------------------------------------------------------ */
...
int main(void)
{
    ...
    // Points directly into AFL++'s shared memory — zero copy, no read()
